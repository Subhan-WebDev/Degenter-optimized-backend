...
async function runBounded(items, limit, fn) {
  ...
  const workers = Array(Math.min(limit, items.length)).fill(0).map(async () => {
    while (true) {
      ...
      try { await fn(items[idx]); } catch (e) { warn('[meta/batch]', items[idx]?.denom || items[idx], e.message); }
    }
  });
...
export async function refreshMetaOnce(denom) {
  if (!denom) return;
  try { await setTokenMetaFromLCD(denom); info('[meta/once] refreshed', denom); }
  catch (e) { warn('[meta/once]', denom, e.message); }
}
...
async function backfillAllTokensOnce() {
  ...
  for (;;) {
    const { rows } = await DB.query(
      `SELECT token_id, denom FROM tokens WHERE token_id > $1 ORDER BY token_id ASC LIMIT $2`,
      [lastId, META_BACKFILL_BATCH]
    );
    if (!rows.length) break;
    const first = rows[0]?.token_id; const last = rows.at(-1)?.token_id;
...
export function startMetaRefresher() {
  (async function loop () {
    try { await backfillAllTokensOnce(); } catch (e) { warn('[meta/backfill]', e.message); }
