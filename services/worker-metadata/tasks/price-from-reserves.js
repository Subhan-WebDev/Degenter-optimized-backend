      ...
      try {
        const { rows } = await DB.query(`
          SELECT p.pool_id, p.pair_contract,
                 b.token_id AS base_token_id, b.denom AS base_denom, b.exponent AS base_exp
          FROM pools p
        ...
        await runBounded(rows, CONCURRENCY, async (r) => {
          try {
            if (r.base_exp == null) { debug('[price/job skip] meta not ready', { pool_id: r.pool_id, denom: r.base_denom }); return; }
            const reserves = await fetchPoolReserves(r.pair_contract);
            const price = priceFromReserves_UZIGQuote({ base_denom: r.base_denom, base_exp: Number(r.base_exp) }, reserves);
          ...
          } catch (e) { warn('[price/reserves]', r.pair_contract, e.message); }
        });
      } catch (e) { warn('[priceFromReserves loop]', e.message); }
