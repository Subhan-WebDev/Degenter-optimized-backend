...
async function getZigUsd() {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    const res = await fetch(url, { headers: { accept: 'application/json', 'X-CMC_PRO_API_KEY': CMC_KEY } });
    if (res.status === 200) {
      const j = await res.json();
      const price = j?.data?.[CMC_SYMBOL]?.quote?.[CMC_CONVERT]?.price;
      if (price == null || !Number.isFinite(Number(price))) throw new Error('CMC returned no price');
      return Number(price);
    }
