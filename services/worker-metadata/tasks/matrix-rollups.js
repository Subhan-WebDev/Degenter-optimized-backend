  ...
  await DB.query(
    ...
    enriched AS (
      SELECT
        ...
        CASE
          WHEN p.is_uzig_quote THEN q.buy_quote_base / 1e6
          ELSE q.buy_quote_base / power(10::numeric, COALESCE(qtk.exponent::int, 6))
        END AS vol_buy_quote,
        CASE
          WHEN p.is_uzig_quote THEN q.sell_quote_base / 1e6
          ELSE q.sell_quote_base / power(10::numeric, COALESCE(qtk.exponent::int, 6))
        END AS vol_sell_quote,
        CASE
          WHEN p.is_uzig_quote THEN (q.buy_quote_base / 1e6)
          ELSE (
            (q.buy_quote_base / power(10::numeric, COALESCE(qtk.exponent::int, 6))) *
            COALESCE(pr.price_in_zig, 0)
          )
        END AS vol_buy_zig,
        CASE
          WHEN p.is_uzig_quote THEN (q.sell_quote_base / 1e6)
          ELSE (
            (q.sell_quote_base / power(10::numeric, COALESCE(qtk.exponent::int, 6))) *
            COALESCE(pr.price_in_zig, 0)
          )
        END AS vol_sell_zig
      FROM q
...
async function rollPoolTVL(label, onlyPoolId = null) {
  await DB.query(
      ...
      SELECT
        ...
        COALESCE(
          (SELECT pr.price_in_zig
           FROM prices pr
           WHERE pr.token_id = p.base_token_id
             AND pr.pool_id  = p.pool_id
           ORDER BY pr.updated_at DESC
           LIMIT 1),
          (SELECT pr2.price_in_zig
           FROM prices pr2
           JOIN pools px ON px.pool_id = pr2.pool_id
           WHERE pr2.token_id = p.base_token_id
             AND px.is_uzig_quote = TRUE
           ORDER BY pr2.updated_at DESC
           LIMIT 1),
          (SELECT o.close
           FROM ohlcv_1m o
           WHERE o.pool_id = p.pool_id
        ...
        CASE
          WHEN p.is_uzig_quote THEN 1::numeric
          ELSE COALESCE(
            (SELECT pr3.price_in_zig
             FROM prices pr3
             JOIN pools py ON py.pool_id = pr3.pool_id
             WHERE pr3.token_id = p.quote_token_id
               AND py.is_uzig_quote = TRUE
             ORDER BY pr3.updated_at DESC
             LIMIT 1),
            NULL
          )
    ...
    SELECT
      pt.pool_id,
      $1,
      COALESCE(pm.vol_buy_quote, 0),
      COALESCE(pm.vol_sell_quote, 0),
      COALESCE(pm.vol_buy_zig, 0),
      COALESCE(pm.vol_sell_zig, 0),
      COALESCE(pm.tx_buy, 0),
      COALESCE(pm.tx_sell, 0),
      COALESCE(pm.unique_traders, 0),
      pt.tvl_zig,
      pt.reserve_base_disp,
      pt.reserve_quote_disp,
      now()
    FROM pool_tvl pt
    LEFT JOIN pool_matrix pm
      ON pm.pool_id = pt.pool_id
     AND pm.bucket  = $1
    ${onlyPoolId ? 'WHERE pt.pool_id = $2' : ''}
    ON CONFLICT (pool_id, bucket) DO UPDATE SET
    ...
    WITH px_from_prices AS (
      SELECT
        ...
        (SELECT pr.price_in_zig
         ...
         WHERE pr.token_id = t.token_id
           AND p2.is_uzig_quote = TRUE
         ORDER BY pr.updated_at DESC
         LIMIT 1) AS price_disp_zig_prices
      FROM tokens t
    ...
    px_from_ohlcv AS (
      SELECT
        ...
        (SELECT AVG(o.close)
         ...
         WHERE p3.base_token_id = t.token_id
           AND p3.is_uzig_quote = TRUE
